...
class WhitelistValidation(Validation):
    def is_valid(self, bundle, request=None):
        ...
        if request.META["REQUEST_METHOD"] == "POST":
            ...
            #TODO fix some uploads still want  validation 
            if url == None:
                print "url is none"
                return
            ...
            if not url.startswith("/media"):
                if not (self.check_domains(url)) and not (url == None):
                    errors = {"url","Url {0} is not allowed! WhitelistValidation".format(url)}
...
def ValidateUrl(request):
    ...
    if not valid:
        data['Error'] = 'Url {0} is not allowed! In Validate URl'.format(url)
